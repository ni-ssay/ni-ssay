...
- **🎞 Video export, without lag** — by default the live browser is **fully native** (no viewport emulation, no screencast — Playwright's live video pipeline makes interaction feel laggy), and a step-by-step `.webm` video is **synthesized after you stop** from the highlighted screenshots, each frame held for as long as the step really took (uses Playwright's bundled ffmpeg automatically). Prefer true full-motion capture? Opt in with `--video` / the Studio checkbox. A Playwright **trace** (`trace.zip`) is always captured.
